...
/** Run with Java 25 and the official 26.2 game libraries. */
public final class RegistrationCompatibilityCheck {
	...
	public static void main(String[] args) throws Exception {
		...
		final String blocks = read(root, "common/src/main/java/mtr/Blocks.java");
		final String items = read(root, "common/src/main/java/mtr/Items.java");
		...
		for (String mapper : new String[]{"BlockMapper", "BlockDirectionalMapper"}) {
			final String source = read(root, "common/src/main/java/mtr/mappings/" + mapper + ".java");
			require(source.contains("super(RegistrationContext.blockProperties(properties));"), mapper + " constructor bypasses registration properties");
		}
		for (String platform : new String[]{"fabric/src/main/java/mtr/MTRFabric.java", "neoforge/src/main/java/mtr/MTRForge.java"}) {
			final String source = read(root, platform);
			require(!source.contains("RegistryUtilities.createItemProperties("), platform + " block items bypass their explicit registration ids");
			require(source.contains("createBlockItemProperties(path, block.get())"), platform + " block items do not preserve custom block names");
			require(!source.contains("new BlockItem("), platform + " block items bypass the tooltip bridge");
		}
		for (String stationColor : new String[]{"BlockStationColor", "BlockStationColorSlab"}) {
			final String source = read(root, "common/src/main/java/mtr/block/" + stationColor + ".java");
			require(source.contains("super(mtr.mappings.RegistrationContext.stationColorProperties(settings));"), stationColor + " lost its vanilla description key");
		}
		final String railwaySign = read(root, "common/src/main/java/mtr/block/BlockRailwaySign.java");
		...
		final String waterItem = read(root, "common/src/main/java/mtr/mappings/PlaceOnWaterBlockItem.java");
		...
		System.out.println("PASS: " + blockIds.size() + " block ids, " + itemIds.size() + " item ids, constructor hooks, property keys, lazy cache, nested/throwing/thread-local context and block-item names");
	}
	...
	private static void verifyIds(String source, Map<String, String> expected) {
		...
		require(actual.equals(expected), "Registration ids do not exactly match MTR.init");
	}
