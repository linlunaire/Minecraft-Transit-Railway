...
public class TicketSystem {
	...
	public static void addObjectivesIfMissing(Level world) {
		try {
			world.getScoreboard().addObjective(BALANCE_OBJECTIVE, ObjectiveCriteria.DUMMY, Text.literal("Balance"), ObjectiveCriteria.RenderType.INTEGER, false, null);
		} catch (Exception ignored) {
		}
		try {
			world.getScoreboard().addObjective(ENTRY_ZONE_OBJECTIVE, ObjectiveCriteria.DUMMY, Text.literal("Entry Zone"), ObjectiveCriteria.RenderType.INTEGER, false, null);
		} catch (Exception ignored) {
		}
	}

	public static ScoreAccess getPlayerScore(Level world, Player player, String objectiveName) {
		return world.getScoreboard().getOrCreatePlayerScore(ScoreHolder.fromGameProfile(player.getGameProfile()), world.getScoreboard().getObjective(objectiveName));
	}
	...
	private static boolean onEnter(Station station, Player player, ScoreAccess balanceScore, ScoreAccess entryZoneScore, boolean remindIfNoRecord) {
		...
		if (entryZone != 0) {
			if (remindIfNoRecord) {
				...
				balanceScore.set(balanceScore.get() - EVASION_FINE);
			}
		}
