...
public class JonTrainSound extends TrainSoundBase {
	...
	public void playNearestCar(Level world, BlockPos pos, int carIndex) {
		...
		if (config.speedSoundCount > 0 && soundId != null) {
			...
			if (floorSpeed > 0) {
				...
				((ClientLevel) world).playLocalSound(pos, RegistryUtilities.createSoundEvent(ResourceLocation.fromNamespaceAndPath(MTR.MOD_ID, speedSoundId)), SoundSource.BLOCKS, 1, 1, false);
			}
		}
	}
	...
	public void playAllCarsDoorOpening(Level world, BlockPos pos, int carIndex) {
		if (world instanceof ClientLevel && config.doorSoundBaseId != null) {
			...
			if (soundId != null) {
				((ClientLevel) world).playLocalSound(pos, RegistryUtilities.createSoundEvent(ResourceLocation.fromNamespaceAndPath(MTR.MOD_ID, soundId)), SoundSource.BLOCKS, 1, 1, false);
			}
		}
	}
