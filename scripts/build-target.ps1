...
try {
    ...
    $previousInternalWrapper = $env:MTR_INTERNAL_GRADLE_WRAPPER
    Push-Location -LiteralPath $projectDirectory
    try {
        if (-not [string]::IsNullOrWhiteSpace($javaHome)) { $env:JAVA_HOME = $javaHome }
        if ($projectDirectory -eq $repositoryRoot) { $env:MTR_INTERNAL_GRADLE_WRAPPER = '1' }
        & $wrapper @gradleArguments
        $buildExitCode = $LASTEXITCODE
    } finally {
        $env:JAVA_HOME = $previousJavaHome
        $env:MTR_INTERNAL_GRADLE_WRAPPER = $previousInternalWrapper
        Pop-Location
    }
    exit $buildExitCode
} catch {
